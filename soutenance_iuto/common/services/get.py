...
class GetAll:
    ...
    def get_all_inscription_soutenance() -> list:
        return InscriptionSoutenance.objects.prefetch_related(
            "soutenance", "prof"
        ).all()
    ...
    def get_all_soutenance() -> list:
        return Soutenance.objects.prefetch_related(
            "stg_alt", "horaire", "salle", "prof_candide"
        ).all()

    @staticmethod
    def get_all_stage_alt() -> list:
        return StageAlt.objects.prefetch_related(
            "entreprise", "tuteur_pro", "tuteur_univ", "etudiant"
        ).all()
...
class GetById:
    ...
    def get_est_dans_promotion_by_etu_id(id: int) -> BaseManager[EstDansPromotion]:
        return (
            EstDansPromotion.objects.prefetch_related("etudiant", "promotion")
            .filter(etudiant=GetById.get_etudiant_by_id(id))
            .all()
        )
    ...
    def get_est_dans_promotion(id_etu: int, id_promo: int) -> EstDansPromotion:
        return (
            EstDansPromotion.objects.prefetch_related("etudiant", "promotion")
            .filter(
                etudiant=GetById.get_etudiant_by_id(id_etu),
                promotion=GetById.get_promotion_by_id(id_promo),
            )
            .first()
        )
    ...
    def get_est_responsable_by_prof_id(id: int) -> list:
        return (
            EstResponsable.objects.prefetch_related("professeur", "promotion")
            .filter(professeur=GetById.get_professeur_by_id(id))
            .all()
        )
    ...
    def get_inscription_soutenance_by_soutenance_id(id: int) -> list:
        return (
            InscriptionSoutenance.objects.prefetch_related("soutenance", "prof")
            .filter(soutenance=GetById.get_soutenance_by_id(id))
            .all()
        )
    ...
    def get_inscription_soutenance_by_prof_id(id: int) -> list:
        return (
            InscriptionSoutenance.objects.prefetch_related("soutenance", "prof")
            .filter(prof=GetById.get_professeur_by_id(id))
            .all()
        )
    ...
    def get_inscription_suivi_by_stg_id(id: int) -> list:
        return (
            InscriptionSuivi.objects.prefetch_related("stg_alt", "prof")
            .filter(stg_alt=GetById.get_stage_alt_by_id(id))
            .all()
        )
...
class Get:
    ...
    def get_etudiants_by_tuteur_pro_id(id: int) -> list:
        return (
            Etudiant.objects.prefetch_related("tuteur_pro")
            .filter(tuteur_pro=GetById.get_tuteur_pro_by_id(id))
            .all()
        )
    ...
    def get_soutenance_by_etu_id(id: int) -> list:
        return (
            Soutenance.objects.prefetch_related(
                "stg_alt", "horaire", "salle", "prof_candide"
            )
            .filter(stg_alt=GetById.get_stage_alt_by_id(id))
            .all()
    ...
    def get_soutenance_by_prof_candide_id(id: int) -> list:
        return (
            ...
            .filter(prof_candide=GetById.get_professeur_by_id(id))
            .all()
        )
    ...
    def get_stage_alt_by_etu_id(id: int) -> BaseManager[StageAlt]:
        return (
            StageAlt.objects.prefetch_related(
                "entreprise", "tuteur_pro", "tuteur_univ", "etudiant"
            )
            .filter(etudiant=GetById.get_etudiant_by_id(id))
            .all()
    ...
    def get_stage_alt_by_tuteur_univ_id(id: int) -> list:
        return (
            ...
            .filter(tuteur_univ=GetById.get_professeur_by_id(id))
            .all()
        )
    ...
    def get_etudiant_by_tuteur_pro_id(id: int) -> list:
        stg_alt = StageAlt.objects.filter(
            tuteur_pro=GetById.get_tuteur_pro_by_id(id)
        ).all()
        print(stg_alt.count())
        list_etu = []
        for stg in stg_alt:
    ...
    def get_all_soutenance_without_candide(id_prof: int) -> list:
        ...
        for sout in GetAll.get_all_soutenance():
            if (
                sout.prof_candide is None
                and sout.stg_alt.tuteur_univ.id_prof != id_prof
            ):
                list_sout.append(sout)
        return list_sout
    ...
    def get_last_id_date_horaire() -> int:
        return (
            DateHoraire.objects.last().id_date_horaire
            if DateHoraire.objects.last()
            else 0
        )
    ...
    @staticmethod
    def get_stg_alt_by_titre_theme_intitule_dt_debut_dt_fin_duree_entreprise_tuteur_pro_tuteur_univ_etudiant(
        titre: str,
        theme: str,
        intitule: str,
        dt_debut: str,
        ...
        tuteur_univ: Professeur,
        etudiant: Etudiant,
    ) -> StageAlt:
        return StageAlt.objects.filter(
            titre_stg_alt=titre,
            theme_stg_alt=theme,
            intitule_env_stg_alt=intitule,
            dt_date_debut_stg_alt=dt_debut,
            dt_date_fin_stg_alt=dt_fin,
            duree_stg_alt=duree,
            entreprise=entreprise,
            tuteur_pro=tuteur_pro,
            tuteur_univ=tuteur_univ,
            etudiant=etudiant,
        ).first()
    ...
    @staticmethod
    def get_inscritpion_suivi_by_prof_id_stage_id(
        id_prof: int, id_stage: int
    ) -> InscriptionSuivi:
        return InscriptionSuivi.objects.filter(
            prof=GetById.get_professeur_by_id(id_prof),
            stg_alt=GetById.get_stage_alt_by_id(id_stage),
        ).first()

    @staticmethod
    def get_soutenance_without_planned_date_tutored_by_prof_id(id_prof: int) -> list:
        list_soutenance = []
        for soutenance in GetAll.get_all_soutenance():
            if ( soutenance.stg_alt.tuteur_univ.id_prof == id_prof and soutenance.horaire is not None):
                if ( soutenance.horaire.heure is None and soutenance.horaire.dt_date is None):
                    list_soutenance.append(soutenance)
            elif (soutenance.horaire is None and soutenance.stg_alt.tuteur_univ.id_prof == id_prof):
                list_soutenance.append(soutenance)
        return list_soutenance
